#include<stdio.h>
#include<Windows.h>
#include<tchar.h>  // TCHAR字符串操作函数  头文件
#include"resource.h"

//文本框句柄
HWND hRes;			//第一个文本句柄
HWND hBufHandle[2]; //缓冲区句柄
HWND hTxt[4];		//消费者句柄
...
DWORD WINAPI ThreadProcProd(LPVOID lpParameter)
{
	...
	for (DWORD i = 0; i < len; i++)  //依次遍历文本框字符串
	{
		...
		TCHAR szTemp[2];
		memset(szTemp, 0, 2 * sizeof(TCHAR));
		// szBuffer + i : szBuffer字符串首地址 + 下标
		memcpy(szTemp, szBuffer + i, sizeof(TCHAR)); //字符拷贝到临时缓存区

		//循环判断，只要其中一个为0则将字符写入
		for (int j = 0; j < 2; j++)
		{
			memset(szBuf, 0, 256 * sizeof(TCHAR));
			::GetWindowText(hBufHandle[j], szBuf, 256 * sizeof(TCHAR));
			if (_tcscmp(szBuf, TEXT("0")) == 0) //如果有有一个缓冲区为0则添加
			{
				::SetWindowText(hBufHandle[j], szTemp); 
				break;
			}
		}
		::ReleaseSemaphore(hSemaphoreCust, 1, NULL); //每次放完之后释放一个 消费者信号量
		LeaveCriticalSection(&cs); //释放令牌
	}
	return 0;
}
...
{
	DWORD index = (DWORD)lpParameter; //线程数组的下标
	while (TRUE)
	{
		::WaitForSingleObject(hSemaphoreCust, INFINITE);// 等待消费者信号量
		EnterCriticalSection(&cs);	//获取令牌

		//从缓冲区中读取字母
		TCHAR szTemp[2];
		memset(szTemp, 0, 2 * sizeof(TCHAR));
		for (int i = 0; i < 2; i++)		//有两个缓存区，其中一个不为 0 就从哪里读取
		{
			::GetWindowText(hBufHandle[i], szTemp, 2);
			if (_tcscmp(szTemp, TEXT("0")) != 0)    //判断两个缓冲区 那个不为0 就从那个读
			{
				Sleep(1000);
				//写入自己的文本框
				TCHAR szBuffer[256];
				...
				memcpy(buf + 1, szTemp, sizeof(TCHAR));
				memcpy(buf + 2, szTemp, sizeof(TCHAR));
				::SetWindowText(hTxt[index], buf);

				//缓冲区置0
				::SetWindowText(hBufHandle[i], TEXT("0"));
				Sleep(1000);
...
BOOL CALLBACK MainDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		//缓冲区
		hBufHandle[0] = GetDlgItem(hDlg, IDC_EDIT_2);
		hBufHandle[1] = GetDlgItem(hDlg, IDC_EDIT_3);
		...
		::SetWindowText(hBufHandle[0], TEXT("0"));
		::SetWindowText(hBufHandle[1], TEXT("0"));
